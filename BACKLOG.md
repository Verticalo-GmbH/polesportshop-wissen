...
2. **Von Pauschale zu historischen Daten (B68) — Buchhaltungs-Anbindung:** GLD-Aufschlag aktuell pauschal (EU 1,00 € / **Nicht-EU 5,00 €**, 2026-06-29 von 2,75 angehoben wg. ~12 % Zoll bei OKSA/VAE) + **feste konservative Fremdwährungskurse** (USD 0,95 / AUD 0,65, `constants.FX_KONSERVATIV`, statt Tageskurs — bewusst „schlechter Kurs" als Puffer). Ziel: die **echten Beschaffungs-Zusatzkosten je Lieferant** (Zoll + Versand + Bank) aus den **historischen Rechnungen** ableiten — über eine **Anbindung an ein Buchhaltungssystem**, in dem die Lieferanten-Rechnungen sichtbar/auswertbar sind. Dann GLD-Aufschlag = realer Lieferanten-Mittelwert statt Pauschale (VK folgt automatisch). Spätestens dann klären, ob der Aufschlag dauerhaft in den GLD oder in eine separate Kostenstelle gehört. Ziel: die **echten Beschaffungs-Zusatzkosten je Lieferant** (Zoll + Versand + Bank) aus den **historischen Rechnungen** ableiten — über eine **Anbindung an ein Buchhaltungssystem**, in dem die Lieferanten-Rechnungen sichtbar/auswertbar sind. Dann GLD-Aufschlag = realer Lieferanten-Mittelwert statt Pauschale (VK folgt automatisch). Spätestens dann klären, ob der Aufschlag dauerhaft in den GLD oder in eine separate Kostenstelle gehört.
