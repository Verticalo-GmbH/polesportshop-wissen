...
| E100 | CRAWLING-DATEN | Vorab vergebene B-Nummern `<PREFIX>-<JAHR>-<NN>` pro Lieferant (Pre-Order-Identifier): an Lieferant + auf Etikett/Dokumente; **führend** in jeder BE-Referenz (`Bestellung <B-Nr> | Rechnung #<Nr> | <Quelle>`). Löst Wareneingangs-Zuordnung bei parallelen Bestellungen + Indent-Orders (B-Nummer jetzt, Artikel/BE bei Ware+EK). Records unter `pipeline/orders/` |
